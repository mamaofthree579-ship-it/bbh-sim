...
<div id="ui">
  ...
  <button id="exportBtn">Export (Placeholder)</button>
</div>
...
function animate() {
    ...
    nodes.forEach(node => {
        node.update(DT);
        const amp = Math.max(0.1, node.amplitudeAt(t));
        node.mesh.scale.set(amp+0.5, amp+0.5, amp+0.5);
        node.mesh.material.color.setHSL(Math.min(1, Math.max(0, (amp+1)/2)), 1, 0.5);

        node.subNodes.forEach(sn => {
            let ampSN = Math.max(0.1, sn.amplitudeAt(t));
