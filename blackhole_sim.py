...
st.set_page_config(page_title="Live Black Hole Simulator", layout="wide")

st.title("🌌 Black Hole Simulator — Live Mode with Chirp")

st.markdown("""
Explore a **3D Schwarzschild black hole** with a rotating plasma hotspot and corresponding **gravitational-wave chirp**.  
Toggle **Live Mode** to watch the orbit and waveform evolve in real time.
""")
...
live_mode = st.sidebar.checkbox("🔄 Live Mode (continuous animation)", value=False)
...
def render_scene(t_phase):
    ...
    # --- Combine ---
    fig = go.Figure(data=[disk, horizon, trail, hotspot])
    fig.update_layout(
        scene=dict(
...
# --- Chirp waveform ---
def make_chirp(t_phase):
    t = np.linspace(0, 1, 2000)
    base_freq = 30 + 20 * np.sin(2 * np.pi * t_phase)
    freq = base_freq * (1 + 15 * t**3) * (1 + speed_factor)
    amp = t**2 * (0.6 + 0.4 * np.sin(2 * np.pi * t_phase))
    signal = amp * np.sin(2 * np.pi * freq * t)
    return t, signal, freq, amp

# --- Render live or static ---
if not live_mode:
    t_phase = st.slider("Orbital phase", 0.0, 1.0, 0.0, step=0.01)
    fig3d = render_scene(t_phase)
    st.plotly_chart(fig3d, use_container_width=True)

    t, chirp_signal, freq, amp = make_chirp(t_phase)
    fig_chirp = go.Figure()
    fig_chirp.add_trace(go.Scatter(x=t, y=chirp_signal, mode="lines", line=dict(color="aqua", width=2)))
    fig_chirp.update_layout(
        paper_bgcolor="black", plot_bgcolor="black",
        xaxis=dict(title="Time (normalized)"),
        yaxis=dict(title="Strain (a.u.)"),
        font=dict(color="white"), margin=dict(l=30, r=30, t=30, b=30)
    )
    ...
    phase = 0.0
    while True:
        fig3d = render_scene(phase)
        placeholder.plotly_chart(fig3d, use_container_width=True)
        phase += 0.01
        if phase >= 1.0:
            phase = 0
        time.sleep(0.1)
        if not st.session_state.get("live_mode_running", True):
            break

# --- Chirp audio ---
t, chirp_signal, freq, amp = make_chirp(0)
sample_rate = 44100
